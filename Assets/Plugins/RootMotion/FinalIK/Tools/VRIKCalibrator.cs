...
namespace RootMotion.FinalIK
{
    ...
    public static class VRIKCalibrator
    {
        ...
        public static CalibrationData Calibrate(VRIK ik, Settings settings, Transform headTracker, Transform bodyTracker = null, Transform leftHandTracker = null, Transform rightHandTracker = null, Transform leftFootTracker = null, Transform rightFootTracker = null)
        {
            ...
            // 頭のトラッカーの位置を基準に、新しい頭(アバターの)の位置を計算
            ...
            /// 
            /// Head
            // headTarget には、新しい or 既存のターゲットが入る
            ...
            //  headTarget を HMD (headTracker) の子オブジェクトにする これがいらないのでは？  →　いらなかった
            // headTarget.parent = headTracker;
            //  VRIK の 頭のターゲット (ik.solver.spine.headTarget) を headTarget にセット
            ik.solver.spine.headTarget = headTarget;

            ///
            /// 
            /// Size
            // ユーザーの頭の高さとアバターの頭の高さを比較して、スケールを適切に変更する
            ...
            // デバッグ
            Debug.Log($"headTarget.position.y: {headTarget.position.y}");
            Debug.Log($"ik.references.root.position.y: {ik.references.root.position.y}");
            Debug.Log($"ik.references.head.position.y: {ik.references.head.position.y}");
            Debug.Log($"Calculated sizeF: {sizeF}");
            ...
            if (bodyTracker != null)
            {
                ...
                ik.solver.plantFeet = false;        // (1) 足を固定しない設定
                ik.solver.spine.maxRootAngle = 180f;     // (2) ルート（腰）が最大180度まで回転可能
            }

            // 足のトラッカーが両方ある場合
            else if (leftFootTracker != null && rightFootTracker != null)
            {
                ik.solver.spine.maxRootAngle = 0f;     // (3) ルートの回転を制限（固定）
            }
            ...
            if (leftHandTracker != null)
            {
                ...
                // 手の向きを決めるための補助ベクトル
                // wristToPalmAxis（手首→手のひら方向）と palmToThumbAxis（手のひら→親指方向）をクロス積（直交ベクトル）で求める
                Vector3 leftHandUp = Vector3.Cross(ik.solver.leftArm.wristToPalmAxis, ik.solver.leftArm.palmToThumbAxis);

                // QuaTools.MatchRotation() → leftHandTracker.rotation の回転を、VRIKの手の向きに合わせる
                ...
                // leftHandTarget.parent = leftHandTracker;   // 同様いらない
            ...
            /// 
            /// Root controller
            // bodyTracker（腰トラッカー）がある または leftFootTracker & rightFootTracker がある場合、addRootController = true
            // 腰トラッカーがある場合、腰の高さをトラッカーに追従させる
            // 足トラッカーがある場合、足の位置を正しく地面と合わせる
            // 地面との接触を考慮しながら、自然な動きに調整
            ...
            if (addRootController)
            {
                ...
                // VRIKのルート位置を調整 する (腰、足のトラッカーがある場合)
            ...
            ik.solver.spine.minHeadHeight = 0f;    // 頭の高さの最小値を無効化
            ik.solver.locomotion.weight = bodyTracker == null && leftFootTracker == null && rightFootTracker == null ? 1f : 0f;   // 足のトラッキングがない場合、移動 (locomotion) を有効化
            ...
            data.leftLegGoal = new CalibrationData.Target(ik.solver.leftLeg.bendGoal);    // 足の曲がり具合（bendGoal）を保存
        ...
        private static void CalibrateLeg(Settings settings, Transform tracker, IKSolverVR.Leg leg, Transform lastBone, Vector3 rootForward, bool isLeft)
        {
            ...
            // rootForward  アバターのルート（胴体）の前方方向ベクトル 
            ...
            f.y = 0f;  // Y軸を 0 にして、水平方向の回転だけにする
            trackerSpace = Quaternion.LookRotation(f);

            // Target position
            // inwardOffset で 左右の位置を微調整
            // footForwardOffset で 前後位置を調整
            // 内向きオフセット（左右の調整）
